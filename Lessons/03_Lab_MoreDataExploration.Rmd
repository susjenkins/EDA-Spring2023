...
# Vectors ---- 
# Vectors are one dimension, you just keep adding lines of value
...
vector1[3] # adding a colon with pull everything from the third item on in vector 1

# Matrices ----
#just like a vector but instead of one line of values you have two, so you need to specify which row/column it's in
# the number of rows and columns has to align with the number in your matrix
#
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) # 
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) # this is running by row (??) 
matrix2
...
matrix1[4, ] # this gives you the fourth row, and you don't specify columns so it gives you all of the columns in the fourth row
matrix1[ , 3] #this gives you all columns in 3 (?) 
matrix1[c(12, 14)] # this is an absolute index, it gives 12 and the 14th item in 
matrix1[c(12:14)] # the colon gives it gives 12 through the 14th item  
matrix1[2:4, 1:3] # things with colon are called 'slices' second through the fourth row and the 1st through the 3rd column
# three variables (cells with four values, one with rnames, one with column names)
cells <- c(1, 26, 24, 68)
rnames <- c("R1", "R2")
cnames <- c("C1", "C2") 
...
#spatial data is a good example of a matrix (with an array?)

# Lists ---- 

#each item in this list of 4 things is given a name
# the semicolon allows you to put two commands in one line in R

list1 <- list(name = "Maria", mynumbers = vector1, mymatrix = matrix1, age = 5.3); list1
list1[[2]]

# Data Frames ---- described as an excel spreadsheet; rows + columns; every row is an observation & every column is what you want to record about the object you are looking at
# size of vector e is 4 even though you only have 3 but you have a missing value placeholder because it needs to be 4 elements long
#since they are all the same length ($) you can put them in the same data frame
...
names(dataframe1) <- c("ID","Color","Passed"); View(dataframe1) # this updates names from d, e, f; if you just run the names as standablone that it just gives you back the names; by viewing the data frame you can name the column vectors and then unltimately refer to them as such

dataframe1[1:2,] # this is a slice because it has the colon in it 
...
> Answer: vectors all go into 'values'; you can think of a dataframe as a list of vectors
every column has to be in the same dataframe 

Question: In the R chunk below, write "dataframe1\$". Press `tab` after you type the dollar sign. What happens?

> Answer: the dollar sign gives you a list of your vector that is refered to by that $[name] 
...
```{r DataExercise-1a}
temp_high_f <- c(64,71,72,63,51,52,61,64,67,73)
temp_low_f <- c(42,49,60,46,32,32,41,47,52,54)
```
...
```{r DataExercise-1b}
c_conversion <- function(temp) { (temp-32)*(5/9) }

c_conversion <- function(temp) { 
  (temp-32)*(5/9) }  
c_conversion(temp_high_f)
```

Combine your four vectors into a data frame and add informative column names.

note: get vector from data frame using $ signs
```{r DataExercise-1c}
...
today <- Sys.Date()
format(today, format = "%B") #capital B means return long day of the month
#format(today, format = "") #google R date formats
#format(today, format = "")
#format(today, format = "")
...
***need to install lubridate package from module recordings before you can run library(lubridate)
...
#Build a function to fix year that is more general than the one discussed in the lesson; this function gives you the date and then the cutoff year? 
fix.early.dates <- function(d, cutoff) {
       m <- year(d) %% 100  #operator %% is a modular division i.e. integer-divide year(d) by 100 and returns the remainder AKA %% gives you the remainder
