...
#paste() command brings together strings; paste0() does not insert spaces
#expression command says this is an expression, not just raw text' is needed because "mu" and "other"R^{}" is not a string; 
...
#creates an output fodler if you do not specify folder/have a folder; this ggsave created an 'Output' folder
...
#Read in data
  ...
  #here() points to what you opened to open this project; a little more stable than setwd

  #here(processeddata >> this inserts path; then do strings as factors)
...
#What kind of values are months? USE CLASS (OR GLIMPSE)
...
factor(PeterPaul.chem.nutrients$month,
  ...
  #month.abb converts number labels to month names 

  #this shows the levels aka the categories found aka the unique (treated as categories)
...
#NEED HELP REDOING LABELS 

  #month.abb >> adds months of the year labels
...
surface_temp_1999_plot <- PeterPaul.chem.nutrients %>% 
  ...
  #running just surface_temp_1999_plot with <- after saves it with all of the following code (???) 
...
#geom is setting point size
#alpha sets the transparency  
#facet wrap adds another dimension to data
