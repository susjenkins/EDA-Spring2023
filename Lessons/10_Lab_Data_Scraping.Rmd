...
```{r pull.and.plot.other.discharge.data}
  ...
  #on url --- '5s' == strings; '15s' == strings with 20 characters; '2-d' = date with 20 characters
  #change site number and period to get any period of gage data if you know the id
...
gage_data <- read.table(theURL,skip = 29,header=TRUE,sep='\t',stringsAsFactors = T) 
  ...
  #SKIP == skipping over rows with text and other misc irrelevant infor for data in the url
...
pcode = '00060' #discharge (cfs) == you had to look up the p code
...
#YOU SHOULD ONLY RUN THE API KEY ONCE
...
net_generation <- the_website %>% html_nodes('td:nth-child(3)') %>% html_text()
total_retail <- the_website %>% html_nodes('td:nth-child(4)') %>% html_text()
the_capacity <- the_website %>% html_nodes('td:nth-child(5)') %>% html_text()
