...
# .shp == shape file (taken as the main file); binary data set so we don't need strings as factors
...
#potential issues with mapView on a mac; send questions in slack but code above when loading packages should solve it
...
wind_raw <- read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv',
  ...
  #colClasses == important when you have a column that's a character vs number; so use colClasses to explicitly set the data type when you read in the column, which works with any column == here we are saying read this column as a factor
  #mutate == saying group by first 8 characters of that factor, and from there you can compute the sum average of wind energy
  #this will get you one row of sum of avg wind energy

  #this is because r would have dropped off first 0 when r reads data if not specified as a factor 
...
huc2_utm <-        st_transform(huc2_sf, crs = 26917)
  ...
  #UTM ZONE 17 == NC; as long as we use this zone for NC data, there will be fewest distortions; you need to find what zone you are in given the data
...
#DONT WORRY ABOUT THE SPARSE, ETC. YOU CAN JUST USE THE FOLLOWING FORMAT
